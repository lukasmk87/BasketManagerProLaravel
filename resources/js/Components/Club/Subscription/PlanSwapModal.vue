            ...
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <h3 class="text-lg font-semibold text-white">
                        {{ trans('subscription.swap.title') }}
                    </h3>
                    <p class="mt-1 text-sm text-blue-100">
                        {{ trans('subscription.swap.preview') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <div v-if="loading" class="space-y-6">
                <div class="flex justify-center items-center py-12">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-600">{{ trans('subscription.swap.calculating') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="error" class="rounded-lg bg-red-50 border border-red-200 p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        <h4 class="text-sm font-semibold text-red-900">{{ trans('subscription.messages.preview_error') }}</h4>
            ...
            <div v-else-if="previewData" class="space-y-6">
                <!-- Plan Comparison -->
                <div>
                    <h4 class="text-sm font-semibold text-gray-900 mb-4">{{ trans('subscription.swap.comparison') }}</h4>
                    <div class="grid grid-cols-2 gap-4">
                        <!-- Current Plan -->
                        <div class="rounded-lg border-2 border-gray-300 bg-gray-50 p-4">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-medium text-gray-600 uppercase">{{ trans('subscription.swap.current') }}</span>
                                <span class="text-xs text-gray-500">{{ currentBillingInterval === 'yearly' ? trans('subscription.billing.yearly') : trans('subscription.billing.monthly') }}</span>
                            </div>
                            ...
                            <p class="text-2xl font-bold text-gray-700">
                                ...
                                <span class="text-sm font-normal text-gray-600">{{ trans(currentBillingInterval === 'yearly' ? 'subscription.billing.per_year' : 'subscription.billing.per_month') }}</span>
                            </p>
                        </div>
                        ...
                        <div class="rounded-lg border-2 p-4" :class="upgradeBorderClass">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-xs font-medium uppercase" :class="upgradeBadgeClass">
                                    {{ isUpgrade ? trans('subscription.swap.upgrade') : isDowngrade ? trans('subscription.swap.downgrade') : trans('subscription.swap.switch') }}
                                </span>
                                <span class="text-xs text-gray-500">{{ billingInterval === 'yearly' ? trans('subscription.billing.yearly') : trans('subscription.billing.monthly') }}</span>
                            </div>
                            ...
                            <p class="text-2xl font-bold" :class="upgradeTextClass">
                                ...
                                <span class="text-sm font-normal text-gray-600">{{ trans(billingInterval === 'yearly' ? 'subscription.billing.per_year' : 'subscription.billing.per_month') }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg bg-blue-50 border-2 border-blue-200 p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <h4 class="text-sm font-semibold text-blue-900 mb-3">{{ trans('subscription.swap.costs') }}</h4>
                            ...
                            <div class="space-y-2 text-sm">
                                ...
                                <div v-if="previewData.proration.credit > 0" class="flex justify-between items-center">
                                    <span class="text-gray-700">
                                        <span class="inline-flex items-center">
                                            ...
                                            {{ trans('subscription.swap.credit') }}
                                        </span>
                                    </span>
                                ...
                                <div class="flex justify-between items-center">
                                    <span class="font-semibold text-gray-900">{{ trans('subscription.swap.due_today') }}:</span>
                            ...
                            <div class="mt-3 pt-3 border-t border-blue-300">
                                <p class="text-xs text-blue-800">
                                    <strong>{{ trans('subscription.swap.what_happens') }}</strong>
                                    <span v-if="isUpgrade">
                                        {{ trans('subscription.swap.explanations.upgrade') }}
                                    </span>
                                    <span v-else-if="isDowngrade">
                                        {{ trans('subscription.swap.explanations.downgrade') }}
                                    </span>
                                    <span v-else>
                                        {{ trans('subscription.swap.explanations.change') }}
                                    </span>
                                </p>
                            </div>
                        </div>
                ...
                <details class="group">
                    <summary class="cursor-pointer rounded-lg border border-gray-300 bg-gray-50 px-4 py-3 hover:bg-gray-100 transition-colors">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-semibold text-gray-900">{{ trans('subscription.swap.details_count', { count: previewData.line_items.length }) }}</span>
                    ...
                    <div class="mt-2 rounded-lg border border-gray-200 overflow-hidden">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">{{ trans('subscription.swap.description') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase">{{ trans('subscription.swap.period') }}</th>
                                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-700 uppercase">{{ trans('subscription.swap.amount') }}</th>
                                </tr>
                            </thead>
                                ...
                                <tr v-for="(item, index) in previewData.line_items" :key="index" :class="item.proration ? 'bg-blue-50' : ''">
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        ...
                                        <span v-if="item.proration" class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ trans('subscription.swap.proration') }}
                                        </span>
                                    </td>
                            ...
                            <tfoot class="bg-gray-50">
                                <tr>
                                    <td colspan="2" class="px-4 py-3 text-sm font-semibold text-gray-900 text-right">{{ trans('subscription.swap.total') }}</td>
                ...
                <div class="rounded-lg bg-gray-100 border border-gray-300 p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h5 class="text-sm font-semibold text-gray-900 mb-1">{{ trans('subscription.swap.next_billing.title') }}</h5>
                            <p class="text-sm text-gray-700">
                                ...
                                {{ trans(billingInterval === 'yearly' ? 'subscription.swap.next_billing.yearly' : 'subscription.swap.next_billing.monthly') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg bg-yellow-50 border-2 border-yellow-200 p-4">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h5 class="text-sm font-semibold text-yellow-900 mb-2">{{ trans('subscription.swap.important_notes.title') }}</h5>
                            <ul class="text-sm text-yellow-800 space-y-1 list-disc list-inside">
                                <li v-html="trans('subscription.swap.important_notes.list.immediate')"></li>
                                <li>{{ trans('subscription.swap.important_notes.list.refund') }}</li>
                                <li>{{ trans('subscription.swap.important_notes.list.payment') }}</li>
                                <li>{{ trans('subscription.swap.important_notes.list.next_billing', { date: formatDate(previewData.next_billing_date) }) }}</li>
                                <li>{{ trans('subscription.swap.important_notes.list.can_change') }}</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-gray-50 px-6 py-4 flex items-center justify-end gap-3">
            ...
            >
                <span v-if="swapping" class="flex items-center">
                    ...
                    {{ trans('subscription.swap.confirming') }}
                </span>
                <span v-else>
                    {{ trans('subscription.swap.confirm') }}
                </span>
            </PrimaryButton>
        </div>
    </Modal>
