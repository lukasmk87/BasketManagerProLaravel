...
const statusConfig = computed(() => {
    const statuses = {
        'draft': {
            label: trans('billing.status.draft'),
            color: 'bg-gray-100 text-gray-800 border-gray-300',
            icon: '📝',
        },
        'open': {
            label: trans('billing.status.open'),
            color: 'bg-yellow-100 text-yellow-800 border-yellow-300',
            icon: '⏳',
        },
        'paid': {
            label: trans('billing.status.paid'),
            color: 'bg-green-100 text-green-800 border-green-300',
            icon: '✓',
        },
        'uncollectible': {
            label: trans('billing.status.uncollectible'),
            color: 'bg-red-100 text-red-800 border-red-300',
            icon: '✕',
        },
        'void': {
            label: trans('billing.status.void'),
            color: 'bg-gray-100 text-gray-800 border-gray-300',
            icon: '∅',
        },
    };
        ...
        <div class="p-6">
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                    <div class="flex items-center space-x-2">
                        <h3 class="text-lg font-semibold text-gray-900">
                            {{ trans('billing.invoices.invoice_number', { number: invoice.number || invoice.id.slice(-8) }) }}
                        </h3>
                        <span
                ...
                <div class="text-right">
                    ...
                    <div v-if="invoice.amount_paid && invoice.amount_paid !== invoice.amount_due" class="text-sm text-gray-600 mt-1">
                        {{ formatAmount(invoice.amount_paid, invoice.currency) }} {{ trans('billing.status.paid').toLowerCase() }}
                    </div>
                </div>
            </div>
            ...
            <div v-if="dueDate" class="mb-4">
                <div class="flex items-center text-sm">
                    ...
                    <span class="text-gray-600">{{ trans('billing.labels.due_on') }}:</span>
                    <span
            ...
            <div v-if="invoice.lines && invoice.lines.length > 0" class="mb-4">
                <div class="text-sm text-gray-600 mb-2">{{ trans('billing.invoices.positions') }}:</div>
                <ul class="space-y-1">
                    <li
                    ...
                    <li v-if="invoice.lines.length > 3" class="text-sm text-gray-500 italic">
                        {{ trans('billing.invoices.more_positions', { count: invoice.lines.length - 3 }) }}
                    </li>
                </ul>
            </div>
            ...
            <div class="flex gap-2 pt-4 border-t border-gray-200">
                ...
                >
                    ...
                    {{ trans('billing.actions.download_pdf') }}
                </SecondaryButton>
            </div>
        </div>
    </div>
