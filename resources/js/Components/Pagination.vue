...
<template>
    <nav v-if="links.length > 3" class="flex items-center justify-between border-t border-gray-200 px-4 sm:px-0">
        <div class="-mt-px flex w-0 flex-1">
            <!-- Previous Link -->
            <template v-for="link in links.slice(0, 1)" :key="link.label">
                <Link
                    v-if="link.url"
                    :href="link.url"
                    class="inline-flex items-center border-t-2 pr-1 pt-4 text-sm font-medium border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700"
                >
                ...
                <span
                    v-else
                    class="inline-flex items-center border-t-2 pr-1 pt-4 text-sm font-medium border-transparent text-gray-300 cursor-not-allowed"
                >
                    <svg class="mr-3 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd" d="M18 10a.75.75 0 01-.75.75H4.66l2.1 1.95a.75.75 0 11-1.02 1.1l-3.5-3.25a.75.75 0 010-1.1l3.5-3.25a.75.75 0 111.02 1.1L4.66 9.25h12.59A.75.75 0 0118 10z" clip-rule="evenodd" />
                    </svg>
        ...
        <div class="-mt-px flex w-0 flex-1 justify-end">
            <!-- Next Link -->
            <template v-for="link in links.slice(-1)" :key="link.label">
                <Link
                    v-if="link.url"
                    :href="link.url"
                ...
                <span
                    v-else
                    class="inline-flex items-center border-t-2 pl-1 pt-4 text-sm font-medium border-transparent text-gray-300 cursor-not-allowed"
                >
                    {{ getDisplayLabel(link) }}
                    <svg class="ml-3 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd" d="M2 10a.75.75 0 01.75-.75h12.59l-2.1-1.95a.75.75 0 111.02-1.1l3.5 3.25a.75.75 0 010 1.1l-3.5 3.25a.75.75 0 11-1.02-1.1l2.1-1.95H2.75A.75.75 0 012 10z" clip-rule="evenodd" />
    ...
    <nav v-if="links.length > 3" class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 md:hidden">
        <div class="flex flex-1 justify-between">
            ...
            <template v-for="link in links.slice(0, 1)" :key="'mobile-' + link.label">
                <Link
                    v-if="link.url"
                    :href="link.url"
                    class="relative inline-flex items-center rounded-md px-3 py-2 text-sm font-medium ring-1 ring-inset ring-gray-300 text-gray-900 hover:bg-gray-50"
                >
                ...
                <span
                    v-else
                    class="relative inline-flex items-center rounded-md px-3 py-2 text-sm font-medium ring-1 ring-inset ring-gray-300 text-gray-400 cursor-not-allowed"
                >
                    <svg class="mr-1 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd" d="M18 10a.75.75 0 01-.75.75H4.66l2.1 1.95a.75.75 0 11-1.02 1.1l-3.5-3.25a.75.75 0 010-1.1l3.5-3.25a.75.75 0 111.02 1.1L4.66 9.25h12.59A.75.75 0 0118 10z" clip-rule="evenodd" />
                    </svg>
            ...
            <!-- Next Link Mobile -->
            <template v-for="link in links.slice(-1)" :key="'mobile-' + link.label">
                <Link
                    v-if="link.url"
                    :href="link.url"
                    class="relative ml-3 inline-flex items-center rounded-md px-3 py-2 text-sm font-medium ring-1 ring-inset ring-gray-300 text-gray-900 hover:bg-gray-50"
                >
                    {{ getDisplayLabel(link) }}
                ...
                <span
                    v-else
                    class="relative ml-3 inline-flex items-center rounded-md px-3 py-2 text-sm font-medium ring-1 ring-inset ring-gray-300 text-gray-400 cursor-not-allowed"
                >
                    {{ getDisplayLabel(link) }}
                    <svg class="ml-1 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        <path fill-rule="evenodd" d="M2 10a.75.75 0 01.75-.75h12.59l-2.1-1.95a.75.75 0 111.02-1.1l3.5 3.25a.75.75 0 010 1.1l-3.5 3.25a.75.75 0 11-1.02-1.1l2.1-1.95H2.75A.75.75 0 012 10z" clip-rule="evenodd" />
